...
export class MenuItemsService {
    ...
    */
    getNestedMenuItems(menuItems: any[], parentMenuItem: { id: any; }) {
        const items = menuItems.filter(item => item.parentId === parentMenuItem.id);
        for (const item of items) {
            item.children = this.getNestedMenuItems(items, item);
        }
        return items;
    }

    async getMenuItems() {
        const menuItems: any = JSON.parse(JSON.stringify(await MenuItem.findAll()));
        for (const menuItem of menuItems) {
            menuItem.children = this.getNestedMenuItems(menuItems, menuItem);
        }
        return menuItems.filter((item: { parentId: number | null; }) => item.parentId === null);
    }
}
